...
export type DataCoverageSource = {
  id: "etf-prices" | "vix" | "cpi" | "fama-french";
  label: string;
  provider: string;
  frequency: "daily" | "monthly";
  /** ISO date of the source's last observation. */
  through: string;
  /** Display form: ISO for daily series, "Month YYYY" for monthly ones. */
  throughLabel: string;
  note: string;
};

/** Per-source freshness boundaries, generated by the research engine. The four
 * sources publish on different calendars, so each carries its own date rather
 * than one shared claim. */
